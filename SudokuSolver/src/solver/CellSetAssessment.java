...
class CellSetAssessment implements Comparable<CellSetAssessment> {
	...
	int couldBeCellCount(Symbol symbol) {
		return m_couldBeCellsForSymbol.get(symbol).size();
	}

	List<Cell> couldBeCellsForSymbol(Symbol symbol) {
	...
	private int ruleOutOtherCellsForSymbol(Cell assignmentCell, Symbol symbol, int stepNumber) {
		int changed = 0;
		Set<Cell> lUnwantedCells = couldBeCellsForSymbol(symbol).stream()
	...
	String couldBeCellsRepresentation(Symbol symbol) {
		return Cell.cellCollectionRepresentation(couldBeCellsForSymbol(symbol));
	}	
}
